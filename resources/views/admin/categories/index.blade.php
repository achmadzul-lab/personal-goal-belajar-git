...
<header class="flex justify-between items-center mb-10">
    <div>
        ...
        <p class="text-slate-500 font-medium">Kelola daftar kategori event di platform ini.</p>
    </div>
...
<div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
    <div class="px-8 py-6 bg-slate-50/50 border-b flex gap-4">
        <form method="GET" action="{{ route('admin.categories.index') }}" class="relative flex-1 flex gap-3">
            ...
            @if($search)
            <a href="{{ route('admin.categories.index') }}" class="px-6 py-3 border border-slate-200 text-slate-600 rounded-xl font-bold hover:bg-slate-50 transition">
                Reset
            </a>
            @endif
        </form>
            ...
            <tbody class="divide-y border-t">
                ...
                <tr class="hover:bg-slate-50/50 transition">
                    ...
                    <td class="px-8 py-5">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center font-bold text-sm uppercase">
                    ...
                    <td class="px-8 py-5 text-sm text-slate-500">{{ $category->updated_at->format('d M Y') }}</td>
                ...
                <tr>
                    <td colspan="5" class="px-8 py-16 text-center text-slate-400 font-medium">
                        @if($search)
                            Tidak ada kategori yang cocok dengan pencarian "<strong>{{ $search }}</strong>".
                        @else
    ...
    <div class="px-8 py-5 bg-slate-50/50 border-t">
        <p class="text-sm text-slate-500 font-medium">Menampilkan {{ $categories->count() }} kategori</p>
    </div>
</div>

{{-- Modal Tambah/Edit --}}
<div id="categoryModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 hidden items-center justify-center p-6">
    <div class="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden">
        <div class="bg-gradient-to-r from-indigo-600 to-indigo-700 px-8 py-6 flex justify-between items-center">
            <h2 id="modalTitle" class="text-xl font-bold text-white">Tambah Kategori</h2>
        ...
        <div class="p-8">
            <form id="categoryForm" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <input id="categoryName" name="name" type="text" placeholder="Contoh: Seminar IT"
...
<div id="deleteModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 hidden items-center justify-center p-6">
    <div class="bg-white w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden">
        ...
        <div class="p-8 text-center">
            ...
            <p class="text-slate-600 mb-1">Yakin ingin menghapus kategori:</p>
